        ...
        time_t* lifetime)
{
    ...
    if (SOAP_OK != soap_call_tns2__getTerminationTime(soap, endpoint.c_str(), NULL, dlg_id_ptr, resp)) {
        char err_buffer[512];
        soap_sprint_fault(soap, err_buffer, sizeof(err_buffer));
        davix_logger(DAVIX_LOG_DEBUG, "Could not retrieve delegated credentials: %s", err_buffer);
        return false;
    }
        ...
        const std::string& ucred, const std::string& dlg_id, int lifetime, bool force, DavixError** err)
{
    ...
    if (force) {
        davix_logger(DAVIX_LOG_VERBOSE, "Renew proxy request");
    ...
    if (certreq.empty()) {
        davix_logger(DAVIX_LOG_VERBOSE, "Get proxy request");
    ...
    if (!certreq.empty()) {
        davix_logger(DAVIX_LOG_VERBOSE, "Sign proxy request");
    ...
    davix_logger(DAVIX_LOG_VERBOSE, "Put new proxy");
         ...
         int lifetime, Davix::DavixError** err)
{
    ...
    if(!get_delegated_credentials(soap, dlg_endpoint, dlg_id, &delegated_lifetime)) {
        davix_logger(DAVIX_LOG_VERBOSE, "No delegated credentials on the storage");
        new_delegation = true;
    }
    else if (delegated_lifetime < lifetime) {
        davix_logger(DAVIX_LOG_VERBOSE, "Need to renew the credentials, %d > %d", lifetime, delegated_lifetime);
        renew_delegation = true;
    }
    else {
        davix_logger(DAVIX_LOG_VERBOSE, "Remaining life of the delegated credentials: %d", delegated_lifetime);
    }
