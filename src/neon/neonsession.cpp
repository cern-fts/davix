...
void NEONSession::authNeonCliCertMapper(void *userdata, ne_session *sess,
    ...
    DAVIX_SLOG(DAVIX_LOG_DEBUG, LOG_CORE, "NEONSession > clicert callback ");
    DavixError::clearError(&(req->_last_error));

    if(params->getClientCertFunctionX509()){
        DAVIX_SLOG(DAVIX_LOG_DEBUG, LOG_CORE, "NEONSession > call client cert callback ");
        SessionInfo infos;
        TRY_DAVIX{
...
int NEONSession::provide_login_passwd_fn(void *userdata, const char *realm, int attempt,
    ...
    DAVIX_SLOG(DAVIX_LOG_DEBUG, LOG_CORE, "NEONSession > Try to get auth/password authentification from client");
     ...
     if(id.first.empty() == false){
         ...
         DAVIX_SLOG(DAVIX_LOG_DEBUG, LOG_CORE, "NEONSession > Try callback for login/passwd for {} time", attempt+1);
         SessionInfo infos;
    ...
    DAVIX_SLOG(DAVIX_LOG_DEBUG, LOG_CORE, "NEONSession > get login/password with success...try server submission ");
    return 0;
...
void configureSession(ne_session *_sess, const Uri & _u, const RequestParams &params, ne_auth_creds lp_callback, void* lp_userdata,
    ...
    if(state == NULL || state != params.getParmState()){
        ...
        DAVIX_SLOG(DAVIX_LOG_TRACE, LOG_CORE, "NEONSession : configure session...");
        ...
        if(params.getSSLCACheck() == false){ // configure ssl check
            DAVIX_SLOG(DAVIX_LOG_DEBUG, LOG_CORE, "NEONRequest : disable ssl verification");
        ...
        if(timespec_isset(params.getConnectionTimeout())){
            ...
            DAVIX_SLOG(DAVIX_LOG_DEBUG, LOG_CORE, "NEONSession : define connection timeout to {}" , timeout);
        ...
        if(timespec_isset(params.getOperationTimeout())){
            ...
            DAVIX_SLOG(DAVIX_LOG_DEBUG, LOG_CORE, "NEONSession : define operation timeout to {}" , timeout);
        ...
        for(std::vector<std::string>::const_iterator it = params.listCertificateAuthorityPath().begin(); it < params.listCertificateAuthorityPath().end(); it++){
            ...
            if ( stat(it->c_str(), &st) < 0 || S_ISDIR(st.st_mode) == false){
                DAVIX_SLOG(DAVIX_LOG_WARNING, LOG_CORE, "NEONSession : CA Path invalid : {}, {} ", *it, ((errno != 0)?strerror(errno):strerror(ENOTDIR)));
                errno = 0;
            }else{
                DAVIX_SLOG(DAVIX_LOG_TRACE, LOG_CORE, " add CA PATH {}", *it);
    ...
    if( params.getClientLoginPassword().first.empty() == false
        ...
        DAVIX_SLOG(DAVIX_LOG_DEBUG, LOG_CORE, "NEONSession : enable login/password authentication");
        ne_set_server_auth(_sess, lp_callback, lp_userdata);
    }else{
        DAVIX_SLOG(DAVIX_LOG_DEBUG, LOG_CORE, "NEONSession : disable login/password authentication");
    }

    // if authentification for cli cert by callback
    if( params.getClientCertFunctionX509()){
        DAVIX_SLOG(DAVIX_LOG_DEBUG, LOG_CORE, "NEONSession : enable client cert authentication by callback ");
        ne_ssl_provide_clicert(_sess, cred_callback, cred_userdata);
    }else{
          DAVIX_SLOG(DAVIX_LOG_DEBUG, LOG_CORE, "NEONSession : disable client cert authentication");
    }
