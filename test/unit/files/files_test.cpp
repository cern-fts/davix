...
TEST(MetalinkReplica, HeaderMetalinkParsing){
    ...
    ret = Davix::Meta::davix_metalink_header_parser(header_key, header_value, origin, u);
    ASSERT_EQ(ret,1);
    ...
    ASSERT_STREQ("http://lxfsra04a04.cern.ch:80/dpm/cern.ch/home/dteam/group?metalink", u.getString().c_str());
     ...
     ret = Davix::Meta::davix_metalink_header_parser(header_key, header_value, origin2, u);
     ASSERT_EQ(ret,1);
     ASSERT_EQ(u.getStatus(), Davix::StatusCode::OK);
     ASSERT_STREQ("https://lxfsra04a04.cern.ch:80/dpm/cern.ch/home/dteam/group?metalink", u.getString().c_str());

     ret = Davix::Meta::davix_metalink_header_parser(header_key, header_value_torrent, origin, u);
     ASSERT_EQ(ret,0);

     ret = Davix::Meta::davix_metalink_header_parser(header_key_location, header_value, origin, u);
     ASSERT_EQ(ret, 0);

     ret = Davix::Meta::davix_metalink_header_parser(header_key_location, header_value_invalid, origin, u);
     ASSERT_EQ(ret, 0);
}
